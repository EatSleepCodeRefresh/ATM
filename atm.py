...
class ATM(Frame):
	...
	def __init__(self, bank):
		"""Initialize the frame, widgets and data model."""
		Frame.__init__(self)
	...
	def _withdraw(self):
		...
		if message:
			...
			self._statusVar.set("Withdraw made")
	...
	def _logout(self):
		...
		self._depositButton["state"] = DISABLED
		self._withdrawButton["state"] = DISABLED
		self._statusVar.set("Have a great day!")
...
def createBank(number = 0):
	"""Saves a bank with the specified number of accounts. Used durting the testing."""
	bank = Bank()
